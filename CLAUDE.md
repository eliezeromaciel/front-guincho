...
| `/planos-implementacoes/` | Implementation plans — never committed |
...
- **Test Strategy by Layer**:
  - **Service Layer (`app/services/*`)**: Test unitarily by mocking the Firestore database (`adminDb` from `firebaseAdmin.server.ts`). Use the global mock helpers (`test/mocks/firebaseAdmin.ts`) to intercept and assert calls.
  - **Route Layer (`app/routes/*`)**: Test loaders and actions directly. Simulate HTTP requests using native `Request` and `FormData` constructors, asserting the returned data/errors. Sessions must be mocked via the global mock helper (`test/mocks/session.ts`).
  - **UI Layer (React Components)**: Test with `happy-dom`, `@testing-library/react`, and `@testing-library/user-event`.
- **Test Conventions**:
  - Test suites (`describe`) and test cases (`it`) must always be written in Portuguese, describing the expected behavior.
  - All test files must reside alongside the code they test (e.g. `app/routes/seguradoras.test.ts`).
  - Global mocks for database and auth are registered in `test/setup.ts` to reduce boilerplate. Use the exported mock functions in tests to alter resolve/reject states.
